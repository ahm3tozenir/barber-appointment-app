...
export abstract class BaseService<T> {
  ...
  getAll(): Observable<DataResponse<T[]>> {
    return this.httpClient.get<DataResponse<T[]>>(
      environment.getApiUrl(`/${this.path}/get-all`)
    );
  }
  getById(id: number): Observable<DataResponse<T>> {
    return this.httpClient.get<DataResponse<T>>(
      environment.getApiUrl(`/${this.path}/get-by-id/` + id)
    );
  }
  create(entity: T | any): Observable<Response> {
    return this.httpClient.post<Response>(
      environment.getApiUrl(`/${this.path}/create`),
      entity
    );
  }
  update(entity: T): Observable<Response> {
    return this.httpClient.post<Response>(
      environment.getApiUrl(`/${this.path}/update`),
      entity
    );
  }
  deleteById(userId: number): Observable<Response> {
    return this.httpClient.delete<Response>(
      environment.getApiUrl(`/${this.path}/delete-by-id/` + userId)
    );
  }
}
